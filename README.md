...
- **Shift+click builds a move-together group**, and moving is the only thing it grants: dragging or arrow-keying any member moves them all as one history entry, every member wears a ring, and the manipulation handles disappear while the group exists so nothing suggests otherwise. Plain deselection dissolves it.
