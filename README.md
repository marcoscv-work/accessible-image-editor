...
- **Flipping and quarter-turn rotation carry every annotation with the picture**, through the exact affine matrix the stage itself renders with, so a redaction that was hiding something keeps hiding it after any number of turns, flipped or not. Under the box kinds the turn folds into their geometry (a rotated rectangle is just its sides swapped); pictures, text and emoji keep it in their own rotation field, because their content has an orientation. One history entry per turn, and four turns return every annotation to its exact original.
- **Straightening** is a free angle in degrees on top of the quarter turns: the image is scaled by exactly the factor needed to keep covering the frame, so no empty corners appear and the crop coordinate space is untouched. Annotations split by what they belong to: a caption belongs to the frame and stays put while the horizon turns underneath it, but a **redaction belongs to the pixels it hides and follows them**, carried through the angle's rotation and cover-scale when the gesture commits.
...
- **Numbers step live**: inside any numeric field the arrow keys (Shift for 10) preview each step on the stage immediately and commit the whole run as one history entry on Enter or blur, because an arrow step is a complete chosen value while "15" on the way to "150" is not. Typed digits stay drafts until confirmed, crop steps pass through the same clamp as commits, and the reducer treats a same-value commit as a no-op.
- **Copy and paste live on the editor's own clipboard**: Ctrl/Cmd+C on a focused annotation, Ctrl/Cmd+V anywhere in the workspace, cascading offsets on repeated pastes, one undo entry each. Deliberately not the system clipboard: no permission prompt, and whatever the user carried in from elsewhere survives.
- **Shift+click builds a move-together group**, seeded from the standing selection, and moving is the only thing it grants: dragging or arrow-keying any member moves them all as one history entry, every member wears a ring, and the manipulation handles disappear while the group exists so nothing suggests otherwise. While the group lives, the layer properties yield to a note rather than offer edits that would apply to one member beside three rings, and a plain click outside the group (or on the background) dissolves it; a plain click on a member keeps it, which is how the group is dragged.
