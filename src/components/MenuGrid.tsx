...
export interface GridChoice {
	...
	/**
	 * What the cell draws. Decoration beside nothing at all, so it is
	 * hidden from assistive technology and `label` is what names the cell.
	 */
	art: React.ReactNode;
...
interface Props {
	...
	/**
	 * How many cells to a row. The grid has to know, because a row is what
	 * the vertical arrows move by.
	 */
	columns: number;
...
/**
 * A picker of drawings rather than words: one cell each, named for the
 * screen reader and titled for the pointer, laid out as a real grid so
 * the arrow keys move in two dimensions (APG grid pattern). One tab stop
...
export function MenuGrid({choices, columns, label, onChoose}: Props) {
	...
	// Opening the picker should offer its first cell, not the one some
	// earlier visit happened to leave behind.
	...
	const handleKeyDown = (event: React.KeyboardEvent) => {
		...
		switch (event.key) {
			...
			case 'ArrowUp':
				...
				// From the top row the up arrow leaves the grid alone, so
				// Clay's own handling can take focus back to the trigger.
