...
/**
 * Every frame is drawn from the crop rectangle at render time rather than
 * baked into geometry of its own, which is what makes it survive the next
 * crop: change the rectangle and the frame is simply drawn again.
 *
 * Its measurements are percentages of the crop's shorter side, so a frame
 * looks the same on a thumbnail card, on the stage at any zoom, and in a
 * full resolution export.
 */
function metrics(crop: CropRect, frame: Frame) {
	const unit = Math.min(crop.width, crop.height) / 100;

	// The two families read the same slider differently: a mat is a band
	// and wants the whole value, a drawn line wants a line's worth of it.
...
/**
 * A rectangle inset from the crop by `inset`, as an SVG rect. Strokes are
 * centered on their path, so a stroked border has to be pulled in by half
 * its width or half of it falls outside the picture.
 */
...
function frameNode(crop: CropRect, frame: Frame) {
	...
	switch (frame.kind) {
		case 'mat':
			return <rect {...common} {...insetRect(crop, gap + band / 2)} strokeWidth={band} />;
		...
		case 'line':
			return <rect {...common} {...insetRect(crop, gap + line / 2)} strokeWidth={line} />;
