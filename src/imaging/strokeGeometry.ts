...
/**
 * Pure geometry for strokes and for the hand-drawn shape style. Nothing
 * here touches the DOM: the preview and the export both consume these
 * paths, which is what keeps them identical.
 */

/**
 * Ramer-Douglas-Peucker simplification over a flat [x, y, ...] list. A
 * freehand gesture reports a point per pointer event, most of which say
 * nothing: what matters is the shape, and the shape survives with a
 * fraction of the points. The epsilon is in image pixels.
 */
...
/**
 * An SVG path through the points: straight segments, or a Catmull-Rom
 * curve converted to cubic beziers, which passes through every point the
 * user placed. A curve that missed its own points would make the pen
 * unpredictable, and predictability is what a keyboard route lives on.
 */
