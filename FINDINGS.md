...
11. **Stacking order is an interaction contract.** The whole-area crop-move surface initially sat above the annotations and swallowed their pointer events (keyboard worked, dragging did not: a bug an all-keyboard test suite cannot see). Pointer-parity e2e coverage was added alongside the fix.
