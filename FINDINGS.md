...
12. **Clay has no small range input.** `form-control-sm`, `input-group-sm`, `form-group-sm` and `panel-sm` all exist, but the slider ships in a single size (no `clay-range-sm`, and `ClaySlider` has no sizing prop), which is a visible gap in a dense tool panel. Scoped CSS gives a thinner track and a 14px visual thumb while the transparent input keeps a 24px height, so the pointer target still meets the minimum. Worth requesting upstream.
