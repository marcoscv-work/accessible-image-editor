...
12. **Clay has no small range input.** `form-control-sm`, `input-group-sm`, `form-group-sm` and `panel-sm` all exist, but the slider ships in a single size (no `clay-range-sm`, and `ClaySlider` has no sizing prop), which is a visible gap in a dense tool panel. Scoped CSS gives a thinner track and a shorter control, while the thumb keeps Clay's 24x24: shrinking it would have made the visible affordance smaller than the minimum target size, which is not a trade worth making for density. Worth requesting a proper `clay-range-sm` upstream.
