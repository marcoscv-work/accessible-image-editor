...
| 2.1.1 Keyboard | A | Pass | Proven by design: the Playwright journeys perform every feature keyboard-only (crop, ratio, zoom, adjustments, annotations, layers, undo/redo, save). Drawing included: the freehand drag falls under this criterion's explicit exemption for path-dependent input, and the keyboard reaches the same stroke through a guided line: start at the crop's centre, arrows aim the end, Enter sets it, arrows bend the middle, Enter finishes, with each stage announced, Backspace stepping back and Escape abandoning. |
